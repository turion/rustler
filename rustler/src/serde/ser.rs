...
use crate::{types::atom, types::tuple, Encoder, Env, OwnedBinary, Term};
use serde::{
    ser::{self, Serialize},
    serde_if_integer128,
};
...
#[inline]
/// Converts a native Rust type into a native Erlang term.
///
/// See [conversion table] for details about serialization behavior.
///
/// [conversion table]: https://github.com/rusterlium/rustler/tree/master/serde_rustler#conversion-table
...
impl<'a> ser::Serializer for Env<'a> {
    type Ok = Term<'a>;
    type Error = Error;
    ...
    fn serialize_bytes(self, v: &[u8]) -> Result<Self::Ok, Self::Error> {
        ...
        Ok(binary.release(self).to_term(self))
    }

    /// Serializes unit (empty tuple) as `nil`.
    #[inline]
    fn serialize_unit(self) -> Result<Self::Ok, Self::Error> {
        Ok(atom::nil().encode(self))
    }
    ...
    fn serialize_unit_variant(
        ...
        atoms::str_to_term(&self, variant).or(Err(Error::InvalidVariantName))
    }
    ...
    where
        T: ?Sized + ser::Serialize,
    {
        let name_term = atoms::str_to_term(&self, name).or(Err(Error::InvalidVariantName))?;
    ...
    fn serialize_struct(
        ...
        let name_term = crate::serde::str_to_term(&self, name).or(Err(Error::InvalidStructName))?;
...
impl<'a> SequenceSerializer<'a> {
    #[inline]
    fn new(env: Env<'a>, len: Option<usize>, name: Option<Term<'a>>) -> Self {
    ...
    fn to_list(&self) -> Result<Term<'a>, Error> {
        Ok(self.items.encode(self.env))
    }

    #[inline]
    fn to_tuple(&self) -> Result<Term<'a>, Error> {
        Ok(tuple::make_tuple(self.env, &self.items))
    }
}
...
impl<'a> ser::SerializeSeq for SequenceSerializer<'a> {
    ...
    where
        T: ?Sized + Serialize,
    {
        self.add(value.serialize(self.env)?);
        Ok(())
    }
...
impl<'a> MapSerializer<'a> {
    ...
    fn to_map(&self) -> Result<Term<'a>, Error> {
        Term::map_from_arrays(self.env, &self.keys, &self.values).or(Err(Error::InvalidMap))
    }

    #[inline]
    fn to_struct(&self) -> Result<Term<'a>, Error> {
        let struct_atom = atom::__struct__().to_term(self.env);
...
impl<'a> ser::SerializeMap for MapSerializer<'a> {
    ...
    where
        T: ?Sized + Serialize,
    {
        self.add_key(key.serialize(self.env)?);
        Ok(())
    }
    ...
    where
        T: ?Sized + Serialize,
    {
        self.add_val(value.serialize(self.env)?);
        Ok(())
    }
...
impl<'a> ser::SerializeStruct for MapSerializer<'a> {
    ...
    where
        T: ?Sized + Serialize,
    {
        let key_term = atoms::str_to_term(&self.env, key).or(Err(Error::InvalidStructKey))?;
