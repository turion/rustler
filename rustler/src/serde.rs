...
use crate::{types::atom, types::tuple, Binary, Decoder, Encoder, Env, Term};
...
/// Checks if the provided term is the atom `nil`.
...
pub fn parse_bool(term: &Term) -> Result<bool, Error> {
    if atom::true_().eq(term) {
        Ok(true)
    } else if atom::false_().eq(term) {
        Ok(false)
    } else {
...
pub fn validate_struct<'a>(term: &Term<'a>, name: Option<&str>) -> Result<Term<'a>, Error> {
    ...
    let __struct__ = atom::__struct__().to_term(term.get_env());
